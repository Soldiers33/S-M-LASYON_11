...
try:
    from dogrulama_testleri import Modul_Gercek_Dunya_Dogrulama_Ek
...
class Simule3_Lab: 
    def __init__(self):
        ...
        if _RESEARCH_READY:
            self.deep_research = Deep_Research_Module(self.const)
        if _DOGRULAMA_HAZIR:
            self.dogrulama_ek = Modul_Gercek_Dunya_Dogrulama_Ek(self.const)
...
class Simule3_Lab_V133(Simule3_Lab):
    ...
    def run_all(self):
        ...
        print(f"\n{Colors.BOLD}{Colors.BLUE}*** AUTONOMOUS DATA & RESEARCH EXPANSION ***{Colors.ENDC}")
        if _NASA_READY:
            self.nasa_live.analiz()
        if _RESEARCH_READY:
            self.deep_research.analiz()
        if _DOGRULAMA_HAZIR:
            self.dogrulama_ek.analiz()
